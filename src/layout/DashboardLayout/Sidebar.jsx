...
const Sidebar = ({ closeSidebar }) => {
  ...
  return (
    <div className="border w-full flex flex-col items-center bg-[#fff] py-[18px] px-[24px] h-full border-l-0 overflow-y-auto overflow-x-hidden border-t-0 border-r-[#E5E5EA]">
      <div className="lg:flex flex-col hidden -ml-[10%] gap-1">
        <img src={Logo} alt="Logo" className="w-[190px] h-[81px]" />
      </div>
      <div
        className={`${location.pathname === '/dashboard' || location.pathname === '/referrals/details' ? 'bg-[#2D84FF]' : ''} flex items-center gap-3 group hover:bg-[#2D84FF] p-2 w-[156px] cursor-pointer rounded-lg h-auto`}
        onClick={() => {
          navigate('/dashboard');
          closeSidebar();
        }}
